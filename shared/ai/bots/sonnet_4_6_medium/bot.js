...
if (typeof window !== 'undefined' && !window.RPSBotAPI) {
    console.error('[sonnet_4_6_medium] bot-api.js must be loaded BEFORE this bot');
}
...
const sonnet46Bot = (() => {
    ...
    function selectMove(gs) {
        ...
        try {
            ...
            console.error('[sonnet_4_6_medium] search error:', e);
        }
    ...
    return {
        ...
        name: 'Sonnet 4.6',
        emoji: '🔷',
        avatar: 'js/bots/sonnet_4_6_medium/avatar-min.png',

        shortDescription: 'ПВС-поиск, байесовская модель и защита от шаттла',
        modelAuthor: 'Anthropic · Claude Sonnet 4.6',
        ...
        tags: ['anthropic', 'claude', 'sonnet', 'pvs', 'bayesian', 'phantom-zone', 'championship'],

        move(gameState) {
            try {
                return selectMove(gameState);
            } catch (e) {
                console.error('[sonnet_4_6_medium] move() error:', e);
                return null;
            }
        },
...
if (typeof RPSBotAPI !== 'undefined' && RPSBotAPI && typeof RPSBotAPI.defineBot === 'function') {
    RPSBotAPI.defineBot(sonnet46Bot);
} else {
    throw new Error('[sonnet_4_6_medium] RPSBotAPI.defineBot is required (load bot-api.js first)');
}
