/**
 * ╔═══════════════════════════════════════════════════════════════════════════╗
 * ║                  OPUS 4.7 FLASH — CLAUDE BATTLE INTELLIGENCE              ║
 * ║                                                                           ║
 * ║  Author: Claude (Anthropic)                                               ║
 * ║  Codename: Opus 4.7 flash                                                 ║
 * ║                                                                           ║
 * ║  Concept:                                                                 ║
 * ║    A patient, paranoid, information-theoretic bot. Combines a rigorous   ║
 * ║    Bayesian opponent model with expected-value attack arithmetic, a       ║
 * ║    time-budgeted alpha-beta search with quiescence over noisy combat,    ║
 * ║    layered flag defense, and an adaptive aggression controller that       ║
 * ║    reads the position and adjusts risk tolerance turn by turn.            ║
 * ║                                                                           ║
 * ║    "Designed by Claude. Plays like Claude: thoughtful first, decisive    ║
 * ║     second. Refuses to lose its flag, refuses to throw away pieces."     ║
 * ║                                                                           ║
 * ║  Architecture:                                                            ║
 * ║    1. Mandatory tactical core   — non-negotiable safety + free wins.     ║
 * ║    2. Critical flag defense     — proactive shield within radius 3.      ║
 * ║    3. High-confidence flag hunt — strike or chase suspect of P >= 0.85.  ║
 * ║    4. EV-driven minimax search  — alpha-beta + quiescence + PV ordering. ║
 * ║    5. Adaptive heuristic pick   — top-K randomized fallback.             ║
 * ╚═══════════════════════════════════════════════════════════════════════════╝
 */

if (typeof window !== 'undefined' && !window.RPSBotAPI) {
    console.error('[opus_4_7_flash] bot-api.js must be loaded BEFORE this bot');
}

const opus47FlashBot = (() => {
    'use strict';
    ...
    const TIME_BUDGET_MS          = 2400;
    const TIME_BUDGET_ENDGAME_MS  = 3600;
    const MAX_DEPTH               = 4;
    const MAX_DEPTH_ENDGAME       = 6;
    const QUIESCENCE_MAX_DEPTH    = 3;
    const BRANCH_LIMIT_MAX        = 16;
    const BRANCH_LIMIT_MIN        = 8;
    const BRANCH_LIMIT_OPP        = 10;

    const HIGH_CONF_FLAG          = 0.85;
    ...
    function move(gameState) {
        try {
            ...
            const adaptive = adaptivePickFromSearch(gameState, available, searched.move);
            if (adaptive) {
                aiEngine.recordAIMove(adaptive);
                return adaptive;
            }
            ...
            console.error('[opus_4_7_flash] move() failed:', e);
            try {
    ...
    return {
        ...
        name: 'Opus 4.7',
        emoji: '💠',
        avatar: 'js/bots/opus_4_7_flash/avatar-min.png',

        shortDescription: 'Байес, EV-атаки и параноидальная защита флага',
        modelAuthor: 'Anthropic · Claude Opus 4.7',
        longDescription: 'Ядро, α-β, байес-EV. Охота на флаг без суицидных разменов.',

        algorithmLabel: 'Тактическое ядро + α-β + байесовский EV',
        tier: 'hard',
        stars: 3,
        difficultyLabel: 'Сложный',
        tags: ['claude', 'anthropic', 'bayesian', 'alpha-beta', 'quiescence',
               'expected-value', 'paranoid-flag', 'championship'],
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = opus47FlashBot;
}

if (typeof RPSBotAPI !== 'undefined' && RPSBotAPI && typeof RPSBotAPI.defineBot === 'function') {
    RPSBotAPI.defineBot(opus47FlashBot);
} else {
    throw new Error('[opus_4_7_flash] RPSBotAPI.defineBot is required (load bot-api.js first)');
}
