...
if (typeof window !== 'undefined' && !window.RPSBotAPI) {
    console.error('[grok_build_0_1] bot-api.js must be loaded BEFORE this bot');
}
...
var grokBuildBot = {
    ...
    name: 'Grok Build',
    emoji: '🛠️',
    avatar: 'js/bots/grok_build_0_1/avatar-min.png',
    ...
    _register() {
        ...
        if (api && typeof api.defineBot === 'function') {
            ...
            throw new Error('[grok_build_0_1] RPSBotAPI.defineBot is required');
        }
    },
    ...
    move(gameState) {
        ...
        try {
            ...
            console.error('[grok_build_0_1] move() crashed:', err);
    ...
    _getMovePool(pieces) {
        var pool = pieces.filter(function (p) {
            return p && !p.immobilized && !grokBuildBot._isPieceBanned(p.id);
        });
    ...
    _pickBestFiltered(moves, gs) {
        ...
        if (this._lastMovedPieceId) {
            var alt = pool.filter(function (m) {
                return m.piece && m.piece.id !== grokBuildBot._lastMovedPieceId;
            });
...
if (typeof RPSBotAPI !== 'undefined' && RPSBotAPI.defineBot) {
    RPSBotAPI.defineBot(grokBuildBot);
} else if (typeof window !== 'undefined' && window.RPSBotAPI && typeof window.RPSBotAPI.defineBot === 'function') {
    window.RPSBotAPI.defineBot(grokBuildBot);
} else {
    throw new Error('[grok_build_0_1] RPSBotAPI is required');
}