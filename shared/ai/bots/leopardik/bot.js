...
if (typeof window !== 'undefined' && !window.RPSBotAPI) {
    console.error('[leopardik] bot-api.js must be loaded before this file');
}
...
const leopardikBot = (() => {
    ...
    return {
        ...
        name: 'Леопардик',
        emoji: '✨',
        avatar: 'js/bots/leopardik/avatar-min.png',
        ...
        tags: ['search', 'beliefs', 'fist-tactics', 'championship'],
        ...
        move(gameState) {
            try {
                ...
                console.error('[leopardik] move() failed:', error);
                const fallback = aiEngine.moveLevel2(gameState);
                if (fallback) {
                    aiEngine.recordAIMove(fallback);
                }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = leopardikBot;
}

if (typeof RPSBotAPI !== 'undefined' && RPSBotAPI && typeof RPSBotAPI.defineBot === 'function') {
    RPSBotAPI.defineBot(leopardikBot);
} else {
    throw new Error('[leopardik] RPSBotAPI.defineBot is required');
}
