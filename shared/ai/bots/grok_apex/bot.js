...
if (typeof window !== 'undefined' && !window.RPSBotAPI) {
    console.error('[grok_apex] bot-api.js must be loaded before this file');
}
...
const grokApexBot = {
    ...
    name: 'Grok Apex',
    emoji: '🧠',
    avatar: 'js/bots/grok_apex/avatar-min.png',
    ...
    modelAuthor: 'xAI · Grok',

    algorithmLabel: 'Байес + тактическое ядро + оценка крепости',
    tier: 'easy',
    stars: 1,
    difficultyLabel: 'Лёгкий',
    tags: ['grok', 'xai', 'independent', 'bayesian', 'original', 'hybrid', 'championship', 'paranoid', 'v3.4'],
    ...
    _register() {
        if (typeof RPSBotAPI !== 'undefined' && RPSBotAPI.defineBot) {
            ...
            throw new Error('[grok_apex] RPSBotAPI.defineBot is required');
        }
    },
