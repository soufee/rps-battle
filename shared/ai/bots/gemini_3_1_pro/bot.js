...
if (typeof window !== 'undefined' && !window.RPSBotAPI) {
    console.error('[gemini_3_1_pro] bot-api.js must be loaded BEFORE this bot');
}

const gemini31ProBot = {
    id: 'gemini_3_1_pro',
    name: 'Gemini 3.1',
    emoji: '✨',
    avatar: 'js/bots/gemini_3_1_pro/avatar-min.png',
    ...
    modelAuthor: 'Google · Gemini 3.1 Pro',
    tier: 'easy',
    stars: 1,
    difficultyLabel: 'Лёгкий',
    tags: ['gemini', 'search', 'adaptive', 'ruthless', 'bayesian', 'champion'],
    ...
    move(gameState) {
        try {
            ...
            console.error('[gemini_3_1_pro] move() failed:', error);
...
if (typeof RPSBotAPI !== 'undefined' && RPSBotAPI.defineBot) {
    RPSBotAPI.defineBot(gemini31ProBot);
} else {
    throw new Error('[gemini_3_1_pro] RPSBotAPI is required');
}
