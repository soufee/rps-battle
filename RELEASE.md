...
**Что завести:** аккаунт на [dev.vk.com](https://dev.vk.com) → «Создать приложение» → тип **Mini App (VK Apps)**.

1. В настройках приложения укажите:
   - **Адрес iframe**: `https://rps-battles.com/`
   - Мобильный адрес: тот же.
2. Скопируйте **«Защищённый ключ»** (Secure key) → `VK_APP_SECRET` в `backend/.env`, перезапустите бэкенд.
3. Проверка: откройте мини-приложение из VK — игра должна автоматически залогинить вас как VK-игрока (без экрана входа). Авторизация идёт через подпись параметров запуска (`sign`), бэкенд проверяет её HMAC-SHA256.
4. Для каталога: заполните название «RPS Battle», описание, иконку 1024×1024 (можно `client/assets/icon.png`), скриншоты, и отправьте на модерацию.

Технически: vk-bridge подключается в `dist/index.html` (скрипт сборки `client/scripts/patch-web-html.js`), клиент шлёт параметры запуска на `/api/v2/auth/vk`.
